...
public class BreadRequestDtoDeserializer extends StdDeserializer<BreadRequestDto> {
    ...
    @Override
    public BreadRequestDto deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JsonProcessingException {
        ObjectMapper mapper = new ObjectMapper();
        ObjectCodec codec = jsonParser.getCodec();
        JsonNode node = codec.readTree(jsonParser);

        List<DayType> dayTypeList = mapper.convertValue(node.get("days"), new TypeReference<List<DayType>>(){});
        BreadState breadState = mapper.convertValue(node.get("state"), new TypeReference<BreadState>(){});
        List<IngredientListResponseDto> ingredients = mapper.convertValue(node.get("ingredientsList"), new TypeReference<List<IngredientListResponseDto>>(){});
