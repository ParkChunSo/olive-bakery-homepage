...
import com.dev.olivebakery.domain.dtos.board.CommentSaveRequestDto;
...
public class BoardService {
  ...
  public BoardService(BoardRepository boardRepository, CommentRepository commentRepository,
      SignService signService, JwtProvider jwtProvider) {
  ...
  public void saveBoard(String bearerToken, PostDetailsRequestDto requestDto) {
    ...
    if (requestDto.isNotice() && !jwtProvider.getUserRolesByToken(bearerToken)
        .contains(MemberRole.ADMIN)) {
  ...
  public void saveComment(String bearerToken, CommentSaveRequestDto comment) {
    Member member = signService
        .findById(jwtProvider.getUserEmailByToken(bearerToken)); //validation check
  ...
  public void updateComment(String bearerToken, CommentRequestDto updateComment) {
    ...
    Comment comment = commentRepository
        .findByUserIdAndUpdateTime(updateComment.getUserId(), updateComment.getUpdateTime())
  ...
  public void deleteComment(String bearerToken, CommentRequestDto deleteComment) {
    ...
    Comment comment = commentRepository
        .findByUserIdAndUpdateTime(deleteComment.getUserId(), deleteComment.getUpdateTime())
        .orElseThrow(() -> new UserDefineException("해당 댓글이 존재하지 않습니다."));
