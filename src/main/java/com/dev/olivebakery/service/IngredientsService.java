...
public class IngredientsService {
  ...
  /* 성분 리스트 -> 성분 디티오 리스트 */
  private List<BreadDto.BreadIngredient> ingredientListToIngredientsDtoList(
      List<Ingredients> ingredientsList) {
    List<BreadDto.BreadIngredient> ingredientDtoList = new ArrayList<>();

    ingredientsList.forEach(ingredients -> {
      ingredientDtoList.add(BreadDto.BreadIngredient.builder().name(ingredients.getName())
          .origin(ingredients.getOrigin()).build());
    });
  ...
  private Ingredients checkIngredientsRepository(Ingredients checkIngredients) {
    return Optional.ofNullable(ingredientsRepository
        .findByNameAndOrigin(checkIngredients.getName(), checkIngredients.getOrigin()))
